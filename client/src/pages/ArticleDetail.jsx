...
export default function ArticleDetail() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) =>
        setRelated(r.data.articles.filter((a) => a.id !== id).slice(0, 3)),
      )
      .catch(console.error)
  ...
  if (!article)
    return (
      <div className="flex flex-col items-center justify-center gap-5 py-32 text-center">
        ...
        <Link
          to="/"
          className="px-6 py-2.5 rounded-full bg-green-600 text-white font-semibold text-sm hover:bg-green-700 transition-colors"
        >
          Về trang chủ
        </Link>
      </div>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 items-start">
        ...
        <article>
          <div className="flex items-center gap-3 mb-4">
            <span
              className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider ${badge}`}
            >
          ...
          <div className="flex gap-3 mt-8 pt-6 border-t border-gray-200">
            <Link
              to="/"
              className="px-4 py-2 rounded-full border-2 border-green-600 text-green-600 text-sm font-semibold hover:bg-green-600 hover:text-white transition-all"
            >
              ← Trang chủ
            </Link>
            <Link
              ...
              <div className="flex flex-col gap-4">
                {related.map((a) => (
                  ...
                  >
                    ...
                    <div>
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${BADGE[a.category] || "bg-green-100 text-green-700"} mb-1`}
                      >
                        {a.categoryLabel}
                      </span>
