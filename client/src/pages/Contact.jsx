...
export default function Contact() {
  ...
  const infos = [
    { icon: '📍', label: 'Địa chỉ', value: 'Tầng 18, Center Building, 01 Nguyễn Huy Tưởng, Hà Nội' },
    { icon: '📞', label: 'Điện thoại', value: '0918.650.033', href: 'tel:0918650033' },
    { icon: '✉️', label: 'Email', value: 'info@digitaltransformation.vn', href: 'mailto:info@digitaltransformation.vn' },
      ...
      <section className="py-16 bg-gradient-to-br from-green-50 via-white to-slate-50 border-b border-gray-100">
        <div className="max-w-[1200px] mx-auto px-6 text-center">
          ...
          <p className="text-gray-500 max-w-xl mx-auto text-base leading-relaxed">
            Đăng ký tư vấn miễn phí — chuyên gia của chúng tôi sẽ phân tích hiện trạng và đề xuất lộ trình phù hợp.
          </p>
        </div>
      </section>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {infos.map(({ icon, label, value, href }) => (
                <div key={label} className="flex gap-4 items-start p-5 rounded-xl bg-gray-50 hover:bg-green-50 transition-colors">
                  <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-white shadow-sm text-xl shrink-0">{icon}</div>
                  <div>
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-0.5">{label}</p>
                    {href ? (
                      <a href={href} className="text-sm font-medium text-green-600 hover:underline">{value}</a>
                    ) : (
